...
{
    public partial class frmInventorEdit: Form
    {
        ...
        public frmInventorEdit()
        {
            InitializeComponent();
        }
        public frmInventorEdit(string state)
        {
            InitializeComponent();
            State = state;
        }
        ...
        private void frmInventorEdit_Shown(object sender, EventArgs e)
        {
            if (State == "add")
            {
                ...
                this.txtBillID.Text = bll.GetAutoCodeByTableName("IN", "WMS_BillMaster", this.dtpBillDate.Value, "1=1");
        ...
        {
            Common.frmSelect frm = new Common.frmSelect(true, "CMD_Cell", "IsLock=0");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            Commands[0] = "WMS.UpdateInventor";
            if (State == "add")
            {
                Commands[0] = "WMS.InsertInventor";
                if (bll.GetRowCount("WMS_BILLMaster", "BillID='" + BillID + "'") > 0)
                {
                    Logger.Error(BillID + " 盘点单号重复，请输入新编号");
                    return;
                }
            ...
            Commands[2] = "WMS.InsertInventorDetail";
            try
            {
            ...
            {
                Logger.Error("盘点单据保存失败，错误原因:" + ex.Message);
                return;
            }
